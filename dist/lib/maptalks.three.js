...
(function (global, factory) {
    ...
    var maptalks__namespace = _interopNamespace(maptalks);
    var THREE__namespace = _interopNamespace(THREE);
    ...
    class LineSegmentsGeometry extends THREE__namespace.InstancedBufferGeometry {
        ...
        setPositions(array) {
            ...
            var instanceBuffer = new THREE__namespace.InstancedInterleavedBuffer(lineSegments, 6, 1);
        ...
        setColors(array) {
            ...
            var instanceColorBuffer = new THREE__namespace.InstancedInterleavedBuffer(colors, 6, 1);
    ...
    class LineSegments2 extends THREE__namespace.Mesh {
        ...
        computeLineDistances() {
            ...
            var instanceDistanceBuffer = new THREE__namespace.InstancedInterleavedBuffer(lineDistances, 2, 1);
    ...
    (function (module, exports) {
        (function (global, factory) {
            ...
            function isValidDiagonal(a, b) {
                return a.next.i !== b.i && a.prev.i !== b.i && !intersectsPolygon(a, b) && (locallyInside(a, b) && locallyInside(b, a) && middleInside(a, b) && (area(a.prev, a, b.prev) || area(a, b.prev, b)) ||
            ...
            function v3Cross(out, v1, v2) {
                var ax = v1[0], ay = v1[1], az = v1[2], bx = v2[0], by = v2[1], bz = v2[2];
                out[0] = ay * bz - az * by;
                out[1] = az * bx - ax * bz;
                out[2] = ax * by - ay * bx;
                return out;
            ...
            function merge(results) {
                ...
                for (var i = 0, len = results.length; i < len; i++) {
                    var _results$i = results[i], position = _results$i.position, indices = _results$i.indices;
                ...
                var pOffset = 0, pCount = 0, iIdx = 0, uvOffset = 0;
                for (var _i2 = 0, _len = results.length; _i2 < _len; _i2++) {
                    var _results$_i = results[_i2], _position = _results$_i.position, _indices = _results$_i.indices, normal = _results$_i.normal, uv = _results$_i.uv;
            ...
            function generateSideWallUV(uvs, vertices, indexA, indexB, indexC, indexD) {
                var idx1 = indexA * 3, idx2 = indexB * 3, idx3 = indexC * 3, idx4 = indexD * 3;
            ...
            function generateTopAndBottom$1(result, triangles) {
                ...
                for (var i = 0, len = triangles.length; i < len; i += 3) {
                    ...
                    index[i + 2] = c;
                    var idx = len + i;
                    var a1 = count + a, b1 = count + b, c1 = count + c;
                    index[idx] = a1;
                    index[idx + 1] = b1;
                    index[idx + 2] = c1;
                }
            ...
            function generateSides$1(result, options) {
                var points = result.points, index = result.index, polygon = result.polygon, uvs = result.uvs;
                var z = options.depth;
                for (var i = 0, len = polygon.length; i < len; i++) {
                    var ring = polygon[i];
                    var j = 0;
                    ...
                    while (j < len1) {
                        ...
                        var x1 = v1[0], y1 = v1[1], x2 = v2[0], y2 = v2[1];
                        points.push(x1, y1, z, x2, y2, z, x1, y1, 0, x2, y2, 0);
                        var a = idx + 2, b = idx + 3, c = idx, d = idx + 1;
            ...
            function flatVertices(polygon, options) {
                ...
                var len = polygon.length;
                var holes = [], flatVertices = new Float32Array(count * 2), points = [], uvs = [];
                var pOffset = count * 3, uOffset = count * 2;
                var z = options.depth;
            ...
            function isClosedRing(ring) {
                var len = ring.length;
                var _ring$ = ring[0], x1 = _ring$[0], y1 = _ring$[1], _ring = ring[len - 1], x2 = _ring[0], y2 = _ring[1];
                return x1 === x2 && y1 === y2;
            }
            ...
            function generateTopAndBottom(result, options) {
                var z = options.depth;
                var points = [], index = [], uvs = [];
                var leftPoints = result.leftPoints, rightPoints = result.rightPoints;
                var i = 0, len = leftPoints.length;
                while (i < len) {
                    var idx0 = i * 3;
                    var _leftPoints$i = leftPoints[i], x1 = _leftPoints$i[0], y1 = _leftPoints$i[1], z1 = _leftPoints$i[2];
                    points[idx0] = x1;
                    points[idx0 + 1] = y1;
                    points[idx0 + 2] = z + z1;
                    var _rightPoints$i = rightPoints[i], x2 = _rightPoints$i[0], y2 = _rightPoints$i[1], z2 = _rightPoints$i[2];
                    var idx1 = len * 3 + idx0;
                    points[idx1] = x2;
                    points[idx1 + 1] = y2;
                    points[idx1 + 2] = z + z2;
                ...
                while (i < len - 1) {
                    var a1 = i, b1 = i + 1, c1 = a1 + len, d1 = b1 + len;
                    index.push(a1, c1, b1);
                    index.push(c1, d1, b1);
                    var len2 = len * 2;
                    var a2 = i + len2, b2 = a2 + 1, c2 = a2 + len, d2 = b2 + len;
                    index.push(a2, c2, b2);
                    index.push(c2, d2, b2);
                    i++;
                }
            ...
            function generateSides(result, options) {
                var points = result.points, index = result.index, leftPoints = result.leftPoints, rightPoints = result.rightPoints, uvs = result.uvs;
                var z = options.depth;
            ...
            function expandLine(line, options) {
                ...
                var points = [], leftPoints = [], rightPoints = [];
                var len = line.length;
                var i = 0;
                while (i < len - 1) {
                    var _p = line[i], _p2 = line[i + 1];
                    var dy = _p2[1] - _p[1], dx = _p2[0] - _p[0];
                    var _rAngle = 0;
                    ...
                    var _calOffsetPoint = calOffsetPoint(_rRad, radius, _p), _op = _calOffsetPoint[0], _op2 = _calOffsetPoint[1];
                ...
                var p1 = line[len - 2];
                var p2 = line[len - 1];
                var _calOffsetPoint2 = calOffsetPoint(rRad, radius, p2), op1 = _calOffsetPoint2[0], op2 = _calOffsetPoint2[1];
                points.push(op1, op2);
            ...
            function calOffsetPoint(rad, radius, p) {
                var x = p[0], y = p[1];
                var z = p[2] || 0;
                var x1 = Math.cos(rad) * radius, y1 = Math.sin(rad) * radius;
                var p1 = [x + x1, y + y1, z];
                var rad1 = rad += Math.PI;
                var x2 = Math.cos(rad1) * radius, y2 = Math.sin(rad1) * radius;
                var p2 = [x + x2, y + y2, z];
                return [p1, p2];
            }
            ...
            function cylinder(point, options) {
                ...
                var _options = options, radius = _options.radius, height = _options.height;
                ...
                var centerx = point[0], centery = point[1];
                var idx = 0, uIdx = 0;
                var offset = circlePointsLen * 3, uOffset = circlePointsLen * 2;
                var indices = [], uvs = [];
                for (var i = -1; i < radialSegments; i++) {
                    var rad = aRad * i;
                    var x = Math.cos(rad) * radius + centerx, y = Math.sin(rad) * radius + centery;
                    points[idx] = x;
                    points[idx + 1] = y;
                    points[idx + 2] = 0;
                    points[idx + offset] = x;
                ...
                for (var _i2 = 0, len = points.length / 2; _i2 < len - 3; _i2 += 3) {
                    var x1 = points[_i2], y1 = points[_i2 + 1], x2 = points[_i2 + 3], y2 = points[_i2 + 4];
                    ...
                    idx += 4;
                    var u1 = uIdx / circlePointsLen, u2 = (uIdx + 1) / circlePointsLen;
    ...
    var polyExtrude = getDefaultExportFromCjs(polyExtrude$2.exports);
    var polyExtrude$1 = _mergeNamespaces({
        __proto__: null,
        'default': polyExtrude
    ...
    var MergeGeometryUtil = Object.freeze({
        __proto__: null,
    ...
    class ExtrudeLineTrail extends BaseObject {
        constructor(lineString, options, material, layer) {
            ...
            const geometry = new THREE__namespace.BufferGeometry();
            const ps = new Float32Array(MAX_POINTS * 3);
            const norls = new Float32Array(MAX_POINTS * 3);
    ...
    var IdentifyUtil = Object.freeze({
        __proto__: null,
    ...
    class HeatMap extends BaseObject {
        constructor(data, options, material, layer) {
            ...
            for (let i = 0, len = position.length, j = 0, len1 = index.length, m = 0, len2 = colored.data.length, n = 0; i < Math.max(len, len1, len2); i += 3) {
                ...
                if (m < len2) {
                    const r = colored.data[m], g = colored.data[m + 1], b = colored.data[m + 2];
                    const rgb = `rgb(${r},${g},${b})`;
                    color.setStyle(rgb);
                    colors[j] = color.r;
                    colors[j + 1] = color.g;
            ...
            if (!close && i === start) {
                ...
                if (needCheckIntersection) {
                    if (removeIntersections && prevOffsetX != null) {
                        const t = lineIntersection(x1, y1, prevOffsetX, prevOffsetY, x2, y2, offsetX, offsetY, tmpIntersection, 0);
                        if (t >= -1e-2 && t <= 1 + 1e-2) {
    ...
    function addExtrudeSide(out, { vertices, topVertices, splittedMap, depth, rect }, start, end, cursors, opts) {
                ...
                for (let s = 0; s <= bevelSegments * splitBevel; s++) {
                    ...
                    for (let i = 0; i < ringVertexCount; i++) {
                        ...
                        if (s > 0 || k > 0) {
                            ...
                            vLen[i] += Math.sqrt((prevX2 - x) * (prevX2 - x)
                                + (prevY2 - y) * (prevY2 - y)
                                + (prevZ2 - zz) * (prevZ2 - zz));
                        }
    ...
    function convertPolylineToTriangulatedPolygon(polyline, polylineIdx, opts) {
        ...
        const outsideIndicesMap = innerOffsetPolyline(points, outsidePoints, 0, pointCount, 0, -lineWidth / 2, miterLimit, false);
        reversePoints(points, 2, 0, pointCount);
        const insideIndicesMap = innerOffsetPolyline(points, insidePoints, 0, pointCount, 0, -lineWidth / 2, miterLimit, false);
        ...
        const indices = new (polygonVertexCount > 0xffff ? Uint32Array : Uint16Array)(((pointCount - 1) * 2 + (polygonVertexCount - pointCount * 2)) * 3);
        let off = 0;
