...
export const Exca = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./exca-mini.glb', (gltf) => {
    const scale = 1;
    style(gltf, cfg);
...
export const Drill = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./drill-mini.glb', (gltf) => {
    const scale = 1.2;
    style(gltf, cfg);
    gltf.scene.rotation.set(Math.PI / 2, -Math.PI / 2, 0);
...
export const Dozer = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./dozer-mini.glb', (gltf) => {
    const scale = 0.5;
    style(gltf, cfg);
    gltf.scene.rotation.set(Math.PI / 2, Math.PI / 2, 0);
...
export const Dump = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./dump-mini.glb', (gltf) => {
    const scale = 0.7;
    style(gltf, cfg);
    gltf.scene.rotation.set(Math.PI / 2, Math.PI * 2, 0);
...
export const Truck = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./truck-mini.glb', (gltf) => {
    const scale = 0.4;
    style(gltf, cfg);
    gltf.scene.rotation.set(Math.PI / 2, Math.PI / 2, 0);
...
export const Toyota = ({ size = 1, x = 0, y = 0, z = 0 }, cfg = {}) => new Promise((res) => LoadCache('./car-mini.glb', (gltf) => {
    const scale = 1.4;
    style(gltf, cfg);
...
export class Vehicle {
    ...
    constructor({ Truck, Maptalks, Three, fps = 60, buffer = true }) {
    ...
    update_exec = ({ gps = [0, 0, 0], utm = [0, 0, 0], head = 0, dur = 0 }) => {
        try {
            ...
            this.frame = fps > 0 ? setInterval(() => { ups.forEach((tween) => tween && tween.update()); }, fps) : null;
            ...
            if (this.isM) {
                ...
                if (fps > 0) {
                    ups[0] = new TWEEN.Tween(this.prev.map.pos).to(pos, duration).easing(Easing.Quadratic.InOut).onComplete(() => clear()).onUpdate((_pos) => {
                    ...
                    ups[1] = new TWEEN.Tween(this.prev.map.rot).to(rot, duration).easing(Easing.Quadratic.InOut).onComplete(() => clear()).onUpdate((_rot) => {
            ...
            if (this.isT) {
                ...
                if (fps > 0) {
                    ups[2] = new TWEEN.Tween(this.prev.three.pos).to(pos, duration).easing(Easing.Quadratic.InOut).onComplete(() => clear()).onUpdate((_pos) => {
                        this.callback('position-three', { gps: _pos });
                        this.TruckThree.position.fromArray([_pos.x, _pos.y, _pos.z]);
                    }).start();
                    ups[3] = new TWEEN.Tween(this.prev.three.rot).to(rot, duration).easing(Easing.Quadratic.InOut).onComplete(() => clear()).onUpdate((_rot) => {
