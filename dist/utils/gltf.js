...
export const Toyota = ({ size = 1, x = 0, y = 0, z = 0 }) => new Promise((res) => Loader.load('./car-mini.glb', (gltf) => {
    const scale = 1.2;
...
export class Vehicle {
    ...
    update = ({ gps = [0, 0, 0], utm = [0, 0, 0], head = 0 }) => {
        try {
            const ups = [];
            const frame = setInterval(() => ups.forEach((tween) => tween && tween.update()), 1000 / this.fps);
            if (this.isM) {
